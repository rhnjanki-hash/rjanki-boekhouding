...
async function init() {
  ...
  `);
  await pool.query("ALTER TABLE records DROP CONSTRAINT IF EXISTS records_type_check").catch(()=>{});
  // seed first admin
  const { rows } = await pool.query("SELECT COUNT(*)::int AS n FROM users");
  if (rows[0].n === 0) {
    const u = (process.env.ADMIN_USER || "admin").trim().toLowerCase();
    const p = (process.env.ADMIN_PASSWORD || "admin123").trim();
